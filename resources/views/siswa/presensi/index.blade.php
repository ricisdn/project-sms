                            ...
                            <table class="table table-hover table-bordered mb-0" id="presensi-table" style="min-width">
                                ...
                                <tbody>
                                    @foreach ($presensi as $presensi)
                                        <tr>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>{{ $presensi->mapel ? $presensi->mapel->nama_mapel : 'Belum ada Mapel' }}
                                            </td>
                                            <td>{{ $presensi->user ? $presensi->user->name : 'Belum ada Siswa' }}</td>
                                            <td>{{ $presensi->tgl_presensi }}</td>
                                            <td class="text-center">
                                                <span
                                                    class="badge {{ $presensi->status == 'Hadir' ? 'badge-success' : 'badge-danger' }}"
                                                    style="width: 5rem;">
                                                    {{ $presensi->status }}
                                                </span>
                                            </td>
